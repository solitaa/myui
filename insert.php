...
function insertForm($chosenDbName, $chosenTableName)
{
    session_start();
    global $db;
    global $baseURL;
    ...
    if ($table) {
        ...
        require_once(dirname(__FILE__) . "/view/rightDiv/insertDataTableOpenTag.php");
...
function insert($chosenDbName, $chosenTableName){
    global $db;
    global $baseURL;
    $db->exec("use $chosenDbName");
    ...
    if (isset($_POST['value'])) {
        ...
        if ($incorrectDatetime == 0 && $incorrectDate == 0 && $incorrectTime == 0 && $emptyFields == 0) {
            $query = "INSERT INTO " . $chosenTableName . " ";
            $query .= "(" . implode(", ", $fieldNames) . ") ";
            $query .= "VALUES (" . implode(',', array_fill(0, count($newData), '?')) . ")";
            $statement = $db->prepare($query);
            $statement->execute(array_values($newData));
            $id = $chosenTableName . "_ID";
            $id = $db->lastInsertId();
            ...
            $_SESSION['new_data'] = $newData;
            var_dump($_SESSION['new_data']);
            header("location: {$baseURL}insert.php/insertForm/$chosenDbName/$chosenTableName");
            exit();
        }
    }
}
